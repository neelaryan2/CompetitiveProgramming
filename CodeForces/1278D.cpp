...
int main() {
	int n;  read(n); seg.rsz(n); cnt.rsz(n);
	rep(i, n) read(seg[i].fi), read(seg[i].se), lesser[seg[i].fi]++;
	rep(i, N - 1) lesser[i + 1] += lesser[i];
	sort(all(seg));
	repr(i, n) {
		cnt[i] = lesser[seg[i].se] - i - 1 - get(seg[i].se);
		add(seg[i].se);
	}
	if (accumulate(all(cnt), 0) != n - 1) goto no;
	rep(i, n - 1) if (cnt[i] < 1) goto no;
	puts("YES"); return 0;
no:
	puts("NO");
	return 0;
