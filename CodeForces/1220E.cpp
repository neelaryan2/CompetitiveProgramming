...
vl deg, interest, noreturn;
vector<vl> adj;
inline void solve() {
	int n = read(), m = read();
	adj.rsz(n);
	rep(i, n) interest.eb(readl());
	rep(i, m) {
	...
	queue<int> q;
	rep(i, n) {
		deg.eb(sz(adj[i])); noreturn.eb(0);
		if (sz(adj[i]) == 1 && i != home) q.push(i);
	}
	while (!q.empty()) {
		int v = q.front(); q.pop();
		deg[v] = N;
		forv(u, adj[v]) {
			if (deg[u] == N) continue;
			deg[u]--;
			noreturn[u] = max(noreturn[u], noreturn[v] + interest[v]);
			if (deg[u] == 1 && u != home) q.push(u);
		}
	}
	ll treemax = 0, graph = 0;
	rep(i, n) {
		if (deg[i] == N) continue;
		graph += interest[i];
		treemax = max(treemax, noreturn[i]);
	}
	cout << graph + treemax << endl;
}