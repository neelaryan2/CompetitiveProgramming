...
int getmx(int v, int u) {
	int ret = -inf, pr = lca(u, v);
	int _u = u, _v = v;
	for (int i = LOGN - 1; i >= 0; i--)
		if (depth[u] - depth[pr] >= (1 << i)) {
			ret = max(ret, mx[u][i]);
			u = P[u][i];
		}
	for (int i = LOGN - 1; i >= 0; i--)
...
int main() {
	...
	for (auto e : edges) {
		if (mst[e.id]) continue;
		ans[e.id] =  getmx(e.u, e.v);
	}
	for (int i : ans) if (i != -inf) cout << i << endl;
}