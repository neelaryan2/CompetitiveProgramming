...
template<typename T>
T vec_hash (vector<T> const vec) {
	T seed = vec.size();
	for (T i : vec)
		seed ^= i + 0x9e3779b9 + (seed << 6) + (seed >> 2);
	return seed;
}
vector<uint> v, hashes;
vector<pair<int, uint>> seg;
bool kmp(vector<uint> vec, int k) {
	vector<int> lps(2 * k, 0);
	for (int i = 1; i < vec.size(); i++) {
		int j = lps[i - 1];
		while (j > 0 && vec[i] != vec[j]) j = lps[j - 1];
		if (vec[i] == vec[j]) j++;
		lps[i] = j;
	}
	forn(i, k, 2 * k - 2) if (lps[i] >= k) return true;
	return false;
}
inline void solve() {
	int n = read(), m = read();
	...
	hashes.eb(vec_hash(v));
	int k = sz(hashes);
	rep(i, k) hashes.eb(hashes[i]);
	if (kmp(hashes, k)) puts("Yes");
	else puts("No");
}